...
interface ContactClientButtonProps {
  /** ID de l'utilisateur admin connecté */
  adminUserId: string;
  /** ID de l'utilisateur client à contacter */
  clientUserId: string;
  /** Nom affiché du client (prénom nom ou email) */
  clientName: string;
...
export default function ContactClientButton({
  ...
  async function handleSend() {
    ...
    // Réutilise le thread s'il en existe un pour cette demande (race condition safe)
  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          <div className="bg-white rounded-xl shadow-2xl w-full flex flex-col" style={{ maxWidth: 480 }}>
            ...
            <div className="px-6 py-5">
              {sent ? (
                <div className="flex flex-col items-center gap-4 py-4 text-center">
                  ...
                  <div>
                    <p className="text-sm font-bold text-black" style={mFont}>Message envoyé !</p>
                    <p className="text-xs text-[#6B7280] mt-1" style={mFont}>
                      {clientName} recevra votre message dans sa messagerie.
                    </p>
                  </div>
                  <button
                    onClick={() => {
                      handleClose();
                      router.push(`/admin/messages${sentThreadId ? `?thread=${sentThreadId}` : ""}`);
                    }}
                ...
                <div className="flex flex-col gap-4">
                  <textarea
                    ...
                    placeholder={`Votre message à ${clientName}… (Entrée pour envoyer)`}
                    rows={4}
                    autoFocus
