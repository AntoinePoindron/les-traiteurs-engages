...
export default function MessagingLayout({
  ...
  useEffect(() => {
    const channel = supabase.current
      .channel("client-incoming-messages")
      .on(
        "postgres_changes",
        {
          event: "INSERT",
  ...
  async function handleSendMessage() {
    const activePending =
      !selectedThread &&
      pendingThread &&
      selectedThreadId === pendingThread.threadId
        ? pendingThread
        : null;

    if (!messageInput.trim() || (!selectedThread && !activePending) || sending) return;
    const body = messageInput.trim();
    setMessageInput("");
    setSending(true);

    const threadId      = selectedThread?.thread_id      ?? activePending!.threadId;
    const partnerId     = selectedThread?.partner_id      ?? activePending!.partnerId;
    const qrId          = selectedThread?.quote_request_id ?? activePending!.quoteRequestId ?? null;
    const ordId         = selectedThread?.order_id         ?? activePending!.orderId         ?? null;
  ...
  // Thread en attente (nouveau, sans messages encore)
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col gap-0.5">
                        <p
                          ...
                          style={{ fontVariationSettings: "'SOFT' 0, 'WONK' 1" }}
                        >
                          {selectedThread?.company_name ?? activePendingThread!.companyName}
                        </p>
                        <p
                          className="text-sm text-black"
                          style={{ fontFamily: "Marianne, system-ui, sans-serif" }}
                        >
                          {selectedThread?.partner_name ?? activePendingThread!.partnerName}
                        </p>
                      </div>
                    </div>
